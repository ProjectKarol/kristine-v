...
<div class="table-responsive">
	<table class="table table-bordered woocommerce-MyAccount-paymentMethods shop_table_responsive account-payment-methods-table">
		<thead>
			<tr>
						...
						<td class="woocommerce-PaymentMethod woocommerce-PaymentMethod--<?php echo esc_attr( $column_id ); ?> payment-method-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							if ( has_action( 'woocommerce_account_payment_methods_column_' . $column_id ) ) {
								...
								if ( ! empty ( $method['method']['last4'] ) ) {
									echo sprintf( esc_html__( '%s ending in %s', 'khaki' ), esc_html( wc_get_credit_card_type_label( $method['method']['brand'] ) ), esc_html( $method['method']['last4'] ) );
								} else {
...
<div class="nk-info-box bg-main-1"><div class="nk-info-box-icon"><span class="ion-ios-information"></span></div><?php esc_html_e( 'No saved methods found.', 'khaki' ); ?></div>

<?php endif; ?>

<?php do_action( 'woocommerce_after_account_payment_methods', $has_methods ); ?>

<a class="nk-btn nk-btn-lg" href="<?php echo esc_url( wc_get_endpoint_url( 'add-payment-method' ) ); ?>"><?php esc_html_e( 'Add Payment Method', 'khaki' ); ?></a>
