...
!function( $ ) {
  ...
  window.nKTabsView = vc.shortcode_view.extend( {
    ...
    cloneModel: function ( model, parent_id, save_order ) {
      ...
      if ( tag === 'nk_tab' ) {
        _.extend( params,
          { tab_id: + new Date() + '-' + this.$tabs.find( '[data-element-type=nk_tab]' ).length + '-' + Math.floor( Math.random() * 11 ) } );
      }
  ...
  window.nKTabView = window.VcColumnView.extend( {
    ...
    cloneModel: function ( model, parent_id, save_order ) {
      ...
      if ( tag === 'nk_tab' ) {
        _.extend( params,
          { tab_id: + new Date() + '-' + this.$tabs.find( '[data-element_type=nk_tab]' ).length + '-' + Math.floor( Math.random() * 11 ) } );
      }
