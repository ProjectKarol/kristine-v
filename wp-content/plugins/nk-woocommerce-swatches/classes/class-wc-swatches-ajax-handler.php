...
class NKWCS_Ajax_Handler {

    private static $instance;

    public static function register() {
        if ( self::$instance == null ) {
            self::$instance = new NKWCS_Ajax_Handler;
        }
    }
    ...
    public function get_product_variations() {
        ...
        $variations = $this->nkwcs_get_available_variations( $product );
     ...
     * @return array
     */
    private function nkwcs_get_available_variations( $product ) {
        global $wpdb;

        $transient_name = 'nkwcs_av_' . (version_compare(WC()->version, '3.0', ">=") ? $product->get_id() : $product->id);
        ...
        $args = array(
            'post_parent' => version_compare(WC()->version, '3.0', ">=") ? $product->get_id() : $product->id,
        ...
        foreach ( $children as $child ) {
            ...
            if ( apply_filters( 'woocommerce_hide_invisible_variations', false, (version_compare(WC()->version, '3.0', ">=") ? $product->get_id() : $product->id), $variation ) && !$variation->variation_is_visible() ) {
                continue;
            }
    ...
    public function on_deleted_transient( $product_id ) {
        delete_transient( 'nkwcs_av_' . $product_id );
    }

}

NKWCS_Ajax_Handler::register();
