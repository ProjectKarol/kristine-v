...
class NKWCS_Product_Attribute_Images {
     ...
     *
     * @return NKWCS_Product_Attribute_Images
     */
    ...
    public function woocommerce_add_attribute_thumbnail_field() {
        ...
        <div class="sub_field form-field swatch-field swatch-field-image" style="overflow:visible;display:none;">
            <div id="swatch-image" class="<?php echo sanitize_title( $this->meta_key ); ?>-image">
                <label><?php _e( 'Thumbnail', 'nkwcs' ); ?></label>
                ...
                <div style="line-height:60px;">
                    <input type="hidden"  class="upload_image_id" id="product_attribute_<?php echo $this->meta_key; ?>" name="product_attribute_meta[<?php echo $this->meta_key; ?>][image]" />
                    <button type="submit" class="upload_swatch_image_button button"><?php _e( 'Upload/Add image', 'nkwcs' ); ?></button>
                    <button type="submit" class="remove_swatch_image_button button"><?php _e( 'Remove image', 'nkwcs' ); ?></button>
                </div>
    ...
    public function woocommerce_edit_attributre_thumbnail_field( $term, $taxonomy ) {
        ...
        $swatch_term = new NKWCS_Swatch_Term( $this->meta_key, $term->term_id, $taxonomy, false, $this->image_size );
        $image = '';
        ?>

        <tr class="form-field ">
            <th scope="row" valign="top"><label><?php _e( 'Type', 'nkwcs' ); ?></label></th>
            <td>
                <label for="product_attribute_swatchtype_<?php echo $this->meta_key; ?>">Swatch Type</label>
                <select name="product_attribute_meta[<?php echo $this->meta_key; ?>][type]" id="product_attribute_swatchtype_<?php echo $this->meta_key; ?>" class="postform">
                    <option <?php selected( 'none', $swatch_term->get_type() ); ?> value="-1"><?php _e( 'None', 'nkwcs' ); ?></option>
                    <option <?php selected( 'color', $swatch_term->get_type() ); ?> value="color"><?php _e( 'Color Swatch', 'nkwcs' ); ?></option>
                    <option <?php selected( 'image', $swatch_term->get_type() ); ?> value="image"><?php _e( 'Image', 'nkwcs' ); ?></option>
                </select>
        ...
        <tr class="form-field swatch-field swatch-field-color section-color-swatch" style="overflow:visible;<?php echo $style; ?>">
            <th scope="row" valign="top"><label><?php _e( 'Color', 'nkwcs' ); ?></label></th>
            <td>
                <div id="swatch-color" class="<?php echo sanitize_title( $this->meta_key ); ?>-color">
        ...
        <?php $style = $swatch_term->get_type() != 'image' ? 'display:none;' : ''; ?>
        <tr class="form-field sub_field swatch-field swatch-field-image" style="overflow:visible;<?php echo $style; ?>">
            <th scope="row" valign="top"><label><?php _e( 'Photo', 'nkwcs' ); ?></label></th>
            <td>
                <div id="product_attribute_thumbnail_<?php echo $this->meta_key; ?>" style="float:left;margin-right:10px;">
                ...
                <div style="line-height:60px;">
                    <input class="upload_image_id" type="hidden" id="product_attribute_<?php echo $this->meta_key; ?>" name="product_attribute_meta[<?php echo $this->meta_key; ?>][image]" value="<?php echo $swatch_term->get_image_id(); ?>" />
                    <button type="submit" class="upload_swatch_image_button button"><?php _e( 'Upload/Add image', 'nkwcs' ); ?></button>
                    <button type="submit" class="remove_swatch_image_button button"><?php _e( 'Remove image', 'nkwcs' ); ?></button>
                </div>
    ...
    public function woocommerce_attribute_thumbnail_field_save( $term_id, $tt_id, $taxonomy ) {
        if ( isset( $_POST['product_attribute_meta'] ) ) {
            ...
            if ( isset( $metas[$this->meta_key] ) ) {
                ...
                update_woocommerce_term_meta( $term_id, $taxonomy . '_' . $this->meta_key . '_image', $image );
    ...
    public function woocommerce_product_attribute_columns( $columns ) {
        ...
        $new_columns[$this->meta_key] = __( 'Thumbnail', 'nkwcs' );
        unset( $columns['cb'] );
    ...
    public function woocommerce_product_attribute_column( $columns, $column, $id ) {
        if ( $column == $this->meta_key ) :
            $swatch_term = new NKWCS_Swatch_Term( $this->meta_key, $id, $this->taxonomy, false, $this->image_size );
