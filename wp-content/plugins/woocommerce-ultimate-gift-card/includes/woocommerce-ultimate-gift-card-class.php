...
if( !class_exists( 'MWB_WGM_Card_Product' ) )
{
	...
	class MWB_WGM_Card_Product{
		...
		{	
			...
			$mwb_wgm_license_hash_saved = get_option('mwb_wgm_license_hash'.$_SERVER['HTTP_HOST'],$mwb_wgm_license_hash_pre);
			...
			$mwb_wgm_hash_today = md5($_SERVER['HTTP_HOST'].$mwb_wgm_license_plugin.$mwb_wgm_license_key);
		...
		{	
			...
			if($mwb_wgm_new_way_of_pdf == 'yes' && $mwb_wgm_wkhtmltopdf){
				...
				if($mwb_wgm_pdf_template_size == 'A3'){
					$result = exec(MWB_WGM_URL.'wkhtmltox/bin/wkhtmltopdf --page-size A3 --encoding utf-8 '.$url.' '.$uploadDirPath.'/giftcard'.$time.$site_name.'.pdf', $output);
				}
		...
		/**
		 * This function is used for stoping the wp_pointer for being initialized or re-start again
		 *
		 * @name mwb_wgm_wp_pointer_end_section
		 * @author makewebbetter<webmaster@makewebbetter.com>
		 * @link http://www.makewebbetter.com/
		 */
		...
		public function mwb_wgm_activate_license(){
            ...
            if(isset($mwb_res['status'])){    
                if( $mwb_res['status'] == true ){
                    update_option('mwb_wgm_license_hash'.$_SERVER['HTTP_HOST'],$mwb_res['hash']);
                    update_option('mwb_wgm_plugin_name','WooCommerce Ultimate Gift Card');
                    update_option('mwb_wgm_license_key'.$_SERVER['HTTP_HOST'],$mwb_res['mwb_key']);
                    update_option('mwb_wgm_plugin_verified'.$_SERVER['HTTP_HOST'],true);
                	...
                	update_option('mwb_wgm_plugin_verified'.$_SERVER['HTTP_HOST'],false);
