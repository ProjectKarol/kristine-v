...
function mwb_wgm_plugin_get_default_privacy_content() {
  return  '<h2>' . __( 'Store Recipient Details for sending Gift Card','woocommerce-ultimate-gift-card' ) . '</h2>' .
  '<p>' . __( "We store your recipient's email address, recipient's name, gift message, your name so that we can send them again your Gift Card with all proper details has been filled by you at the time of purchasing Gift Card Product if they are arrived","woocommerce-ultimate-gift-card" ) . '</p>';
}
...
function mwb_wgm_plugin_add_suggested_privacy_content() {
  ...
  wp_add_privacy_policy_content( __( 'Woocommerce Ultimate Gift Card','woocommerce-ultimate-gift-card' ), $content );
}
...
function mwb_wgm_plugin_register_exporters( $exporters ) {
  $exporters[] = array(
    'exporter_friendly_name' => __( 'Recipient Details','woocommerce-ultimate-gift-card' ),
...
function mwb_wgm_plugin_user_data_exporter( $email_address, $page = 1 ) {
  ...
  if ( $user && $user->ID ) {
    ...
    $group_label = __( 'Gift Card Details','woocommerce-ultimate-gift-card' );
    		...
    		if(isset($order_id) && !empty($order_id)){
				...
				foreach( $order->get_items() as $item_id => $item )
				{
					...
					if(!empty($item_meta_data)){
						...
						if ( !empty($to) ) {
					      $data[] = array(
					        'name'  => __( 'Recipient Name/Email', 'woocommerce-ultimate-gift-card' ),
					        'value' => $to,
					      );
					    }
					    if ( !empty($to_name) ) {
					      $data[] = array(
					        'name'  => __( 'Recipient Name', 'woocommerce-ultimate-gift-card' ),
					        'value' => $to_name,
					      );
					    }
					    if ( !empty($from) ) {
					      $data[] = array(
					        'name'  => __( 'Buyer Name/Email', 'woocommerce-ultimate-gift-card' ),
					        'value' => $from,
					      );
					    }
					    if ( !empty($gift_msg) ) {
					    ...
					    if ( !empty($gift_img_name) ) {
					      $data[] = array(
					        'name'  => __( 'Uploaded Image', 'woocommerce-ultimate-gift-card' ),
					        'value' => $gift_img_name,
					      );
						    }
					}
...
function mwb_wgm_plugin_register_erasers( $erasers = array() ) {
  $erasers[] = array(
    'eraser_friendly_name' => __( 'Recipient Details','woocommerce-ultimate-gift-card' ),
						...
						foreach ($item_meta_data as $key => $value){
							if(isset($value->key) && $value->key=="To" && !empty($value->value)){
								...
								if($status){
									...
									$messages[] = __( 'Removed key "TO"','woocommerce-ultimate-gift-card');
	      							$items_retained = true;
								}
							}
							if(isset($value->key) && $value->key=="To Name" && !empty($value->value)){
								...
								if($status){
									...
									$messages[] = __( 'Removed key "TO Name"','woocommerce-ultimate-gift-card');
	      							$items_retained = true;
								}
							}
							if(isset($value->key) && $value->key=="From" && !empty($value->value)){
								...
								if($status){
									...
									$messages[] = __( 'Removed key "From"','woocommerce-ultimate-gift-card');
	      							$items_retained = true;
								}
							}
							if(isset($value->key) && $value->key=="Message" && !empty($value->value)){
								...
								if($status){
									...
									$messages[] = __( 'Removed key "Message"','woocommerce-ultimate-gift-card');
	      							$items_retained = true;
								}
							}
							if(isset($value->key) && $value->key=="Image" && !empty($value->value)){
								...
								if($status){
									...
									$messages[] = __( 'Removed key "Image"','woocommerce-ultimate-gift-card');
	      							$items_retained = true;
								}
							}
						}
