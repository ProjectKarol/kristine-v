...
class NK_Instagram extends WP_Widget
{
    ...
    function __construct()
    {
        parent::__construct(
            ...
            esc_html__('(nK) Instagram', 'khaki-widgets'), // Name
            array('description' => esc_html__('Instagram images list.', 'khaki-widgets'),) // Args
        );
    }
    ...
    {
        $title = !empty($instance['title']) ? $instance['title'] : esc_html__('Instagram', 'khaki-widgets');
        $images_count = !empty($instance['images_count']) ? $instance['images_count'] : 6;
        ?>
        <p>
            <label><?php echo esc_html__("Before use, you need to configure access to Instagram here - ", 'khaki-widgets') . '<a target="_blank" href="' . esc_url(admin_url('customize.php?autofocus[section]=khaki_instagram')) . '">' . esc_url(admin_url('customize.php?autofocus[section]=khaki_instagram')) . '</a>'; ?></label>
        </p>
        <p>
        ...
        <p>
            <label
                for="<?php echo esc_attr($this->get_field_id('images_count')); ?>"><?php esc_html_e('Images Count:', 'khaki-widgets'); ?></label>
...
add_action('widgets_init',
    create_function('', 'return register_widget("NK_Instagram");')
);
